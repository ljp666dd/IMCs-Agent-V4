"""
DOS Fingerprint Evaluation for CGCNN V2 Model
Evaluates the 400-dimensional DOS vector with proper denormalization.
"""
...
def evaluate_dos_v2():
    ...
    def collect_dos(loader):
        """Collects true and predicted DOS vectors with denormalization."""
        dos_true, dos_pred = [], []
        mat_ids = []
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                if hasattr(batch, 'y_dos'):
                    ...
                    pred_dos_raw = out['dos'].view(-1, 400) * dos_std + dos_mean
                    dos_pred.extend(pred_dos_raw.detach().numpy().tolist())
    ...
    print("\n" + "=" * 50)
    print("V2 Model - DOS Fingerprint (400-dim) Evaluation")
    print("=" * 50)
    ...
    energy_axis = np.linspace(-5, 5, 400)
    
    # Plot
    fig, axes = plt.subplots(2, 1, figsize=(12, 8))
    
    # Top: Per-dimension R2
    ax1 = axes[0]
    ...
    ax1.set_title("V2 Model - Per-Dimension R2 of DOS Prediction")
    ax1.set_ylim(-1, 1)
    ...
    plt.tight_layout()
    save_path = "data/dos_evaluation_v2_overview.png"
    ...
    plt.tight_layout()
    save_path2 = "data/dos_example_comparisons_v2.png"
