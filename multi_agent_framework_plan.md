...
1. 任务图治理：补齐超时、取消、失败重试与可重放机制  
2. 证据链质量：证据去重、来源可信度分层与评分标准化  
3. HOR 候选筛选链路：Top-N 过滤、多目标排序与规则可配置化  
4. 实验闭环：实验数据上传 → 结构化 → 入库 → 反哺训练  
5. 数据治理与版本：dataset hash、模型注册与可复现实验  
6. 可解释性与可视化：SHAP/d-band/DOS 面板与任务图交互  
